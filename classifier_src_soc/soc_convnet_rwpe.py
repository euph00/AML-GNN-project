...
def main(output_dir='./saved_models/', num_epochs=50, learning_rate=0.001):
    ...
    class_weights = torch.tensor([1.0, 800.0]).to(DEVICE) #place 800x weight on positive examples, since we are doing 1:30 undersampling, feel free to adjust these numbers and experiment yourself
    ######################################################
    ...
    n_class_0_sample = n_class_1 * 1000 # We want to sample 1000 legitimate transactions for every laundering transaction during loss calc
    ...
    # Evaluate
    model.load_state_dict(torch.load(output_dir + 'best_gcn_rwpe_model.pt'))
    model.eval()
    with torch.no_grad():
