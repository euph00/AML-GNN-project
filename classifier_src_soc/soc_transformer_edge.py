...
class graph_MHA_layer(nn.Module):
    ...
    def message_func(self, edges):
        qek = edges.dst['Q'] * edges.data['E'] * edges.src['K']
        qikj = (qek).sum(dim=2).unsqueeze(2)
        scores = qikj / self.scale_factor
        vj = edges.src['V']
        eij = qek / self.scale_factor
        edges.data['e'] = eij
    ...
    def forward(self, g, h, e):
        ...
        g.edata['E'] = E.reshape(-1, self.num_heads, self.per_head_hidden_dim)
...
class GraphTransformer_layer(nn.Module):
    def __init__(self, hidden_dim=128, num_heads=8, dropout=0.1):
        ...
        self.hidden_dim = hidden_dim
        self.num_heads = num_heads
        self.dropout_h_mha = nn.Dropout(dropout)
        self.dropout_h_mlp = nn.Dropout(dropout)
    ...
    def forward(self, g, h, e):
        ...
        h_MHA, e_MHE = self.gMHA(g, h, e)

        h_MHA = h_MHA.reshape(-1, self.hidden_dim)
        h_MHA = self.dropout_h_mha(h_MHA)
        h_MHA = self.WO(h_MHA)
        h = h_residual + h_MHA

        e_MHE = e_MHE.reshape(-1, self.hidden_dim)
        ...
        h_MLP = self.dropout_h_mlp(h_MLP)
        h_MLP = self.linear2(h_MLP)
        h = h_residual + h_MLP

        e_residual = e
        e = self.layer_norm2e(e)
        e_MLP = self.linear1e(e)
        e_MLP = torch.relu(e_MLP)
        e_MLP = self.dropout_e_mlp(e_MLP)
        e_MLP = self.linear2e(e)
        e = e_residual + e_MLP
...
class GraphTransformer_net(nn.Module):
    def __init__(self, input_dim=2, hidden_dim=128, output_dim=2, transformer_layers=6):
        ...
        self.embedding_e = nn.Linear(self.embedding_edge_feats.out_dim, hidden_dim)
        self.MLP_classifier = MLP_classifier(hidden_dim + hidden_dim + hidden_dim, output_dim)
    ...
    def forward(self, g):
        ...
        e = self.embedding_e(edge_feats_embed)
        ...
        edge_embed = torch.cat([src_embed, dst_embed, e], dim=1) # (E, 384) each edge has 384 dim representation made up of its source node, dest node and projected transaction features
...
def main(output_dir='./saved_models/', num_epochs=500, learning_rate=0.001):
    ...
    class_weights = torch.tensor([1.0, 80.0]).to(DEVICE) #place 30x weight on positive examples
    ######################################################
